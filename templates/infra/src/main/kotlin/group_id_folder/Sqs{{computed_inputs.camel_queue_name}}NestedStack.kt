...
class Sqs{{computed_inputs.camel_queue_name}}NestedStack(scope: Construct, id: String, stage: Stage) : NestedStack(scope, id) {
    val queue: Queue = Queue.Builder.create(this, "{{inputs.sqs_consumer_queue_name}}")
        .queueName("{{inputs.sqs_consumer_queue_name}}")
        .build()

    init {
        Manifests.env["SQS_QUEUE_CONSUME"] = EnvValue.fromValue(queue.queueName)
